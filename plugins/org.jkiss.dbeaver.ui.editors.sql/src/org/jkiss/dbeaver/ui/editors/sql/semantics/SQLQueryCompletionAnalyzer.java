...
//        }
    ...
    private DBPImage prepareProposalImage(@NotNull SQLQueryCompletionItem item) {
        return switch (item.getKind()) {
            case SCHEMA, CATALOG, UNKNOWN ->  DBDValueFormatting.getObjectImage(item.getObject());
            ...
            case NEW_TABLE_NAME, USED_TABLE_NAME -> {
                ...
                yield object == null ? DBIcon.TREE_TABLE : DBDValueFormatting.getObjectImage(object);
            }
            ...
            case PROCEDURE -> item.getObject() == null ? DBIcon.TREE_FUNCTION : DBDValueFormatting.getObjectImage(item.getObject());
